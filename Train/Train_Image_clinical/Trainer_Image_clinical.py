...
# @Time    : 4-4-2024  15:24
# @Author  : Yi Chen
# @File    : Trainer_Image_clinical.py
# @Software: PyCharm
# @Describe: 
# -*- encoding:utf-8 -*-
# -*- coding: utf-8 -*-
...
def main(root, batch_size, model_name, Img_path, cut_off, save_path, resize_Shape):
    ...
    for i in range(10):
        ...
        #     OneOf([RandFlip(spatial_axis=(1, 2), prob=0.5),
            ...
            while start:
                ...
                for i, data in enumerate(Bar(train_loader), 1):
                    ...
                    for lens in range(output.shape[0]):
                        out.append(torch.sigmoid(output[lens]).detach().cpu().numpy())
                        label_t.append(label[lens].detach().cpu().numpy())

                    loss = criterion(output, label)
                    acc, sen, spec = caculate(out, label_t, cut_off=cut_off)
                    train_acc += acc
                ...
                for j, data_v in enumerate(Bar(val_loader), 1):
                    with torch.no_grad():
                        ...
                        output_val = model(Img,clin_feature_v)
                        for lens_v in range(output_val.shape[0]):
                            out_v.append(torch.sigmoid(output_val[lens_v]).detach().cpu().numpy())
                            label_v.append(label_val[lens_v].detach().cpu().numpy())
                        loss_v = criterion(output_val, label_val)
                        acc_v, sen_v, spec_v = caculate(out_v, label_v, cut_off=cut_off)
