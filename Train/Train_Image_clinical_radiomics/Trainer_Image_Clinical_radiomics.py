...
# @Time    : 4-4-2024  15:36
# @Author  : Yi Chen
# @File    : Trainer_Image_Clinical_radiomics.py
# @Software: PyCharm
# @Describe: 
# -*- encoding:utf-8 -*-
# -*- coding: utf-8 -*-
...
# @Author  : Yi Chen
# @File    : Image_radiomics_main_Feb10.py
# @Software: PyCharm
# @Describe:
# -*- encoding:utf-8 -*-
# -*- coding: utf-8 -*-
# @Time    : 27-9-2023  16:03
# @Author  : Yi Chen
# @File    : Main_code.py
# @Software: PyCharm
# @Describe:
...
from monai.transforms import (
    Compose, RandRotate90, RandFlip,RandRotate,EnsureChannelFirst, Resize,
    RandZoom, RandGaussianNoise, OneOf, RandAdjustContrast, LoadImage
)
...
from Train_all_in_April.Train_Image_clinical_radiomics.Image_clinical_radiomics_dataloader import All_info_loader, load_all_information, \
    All_info_loader_noBCLC, load_all_information_noBCLC
...
class image_clinical_net(nn.Module):
    def __init__(self, in_Channel, out_channel):
        ...
        self.model1 = monai.networks.nets.resnet50(spatial_dims=3, n_input_channels=in_Channel, num_classes=in_Channel)
        self.extract_image_feature = nn.Sequential(*list(self.model1.children())[:-1])
        self.bn = nn.BatchNorm1d(2048 + 25)
        self.model2 = nn.Sequential(
            nn.Linear(2048 + 25, 512),
...
class image_clinical_net_v2(nn.Module):
    def __init__(self, in_Channel, middle_channel, out_channel):
        super().__init__()
        import monai
        self.outc = out_channel
        self.model1 = monai.networks.nets.resnet50(spatial_dims=3, n_input_channels=in_Channel,
                                                   num_classes=middle_channel)
        self.bn = nn.BatchNorm1d(middle_channel + 25)
        self.model2 = nn.Sequential(
            nn.Linear(middle_channel + 25, 16),
            nn.ReLU(inplace=True),
...
class image_clinical_net_v2_noBCLC(nn.Module):
    def __init__(self, in_Channel, middle_channel, out_channel):
        super().__init__()
        import monai
        self.outc = out_channel
        self.model1 = monai.networks.nets.resnet50(spatial_dims=3, n_input_channels=in_Channel,
                                                   num_classes=middle_channel)
        self.bn = nn.BatchNorm1d(middle_channel + 20)
        self.model2 = nn.Sequential(
            nn.Linear(middle_channel + 20, 16),
            nn.ReLU(inplace=True),
            nn.Linear(16, out_channel))
...
def caculate_torch(pred, labels, cut_off):
...
def caculate(pred, labels, cut_off):
    ...
    binary_predictions = np.where(pred >= cut_off, 1, 0)
    assert pred.shape == labels.shape, 'check data shape'

    TP = np.sum((binary_predictions == 1) & (labels == 1)).item()
    TN = np.sum((binary_predictions == 0) & (labels == 0)).item()
    FP = np.sum((binary_predictions == 1) & (labels == 0)).item()
    FN = np.sum((binary_predictions == 0) & (labels == 1)).item()
    ...
    if (TN == 0) & (FP == 0):
        ...
        specificity = TN / (TN + FP)
    fpr_t, tpr_t, _ = roc_curve(labels, pred, pos_label=1)
    roc_auc_t = auc(fpr_t, tpr_t)
    return accuracy, sensitivity, specificity, roc_auc_t
...
def main(root, batch_size, model_name, Img_path, cut_off, save_path, resize_Shape):
    ...
    for i in range(10):
        ...
        Load_feature_t, all_labels_t, patient_file_t = load_all_information_noBCLC(root, train_patient_list)
        Load_feature_v, all_labels_v, patient_file_v = load_all_information_noBCLC(root, val_patient_list)
        ...
        # ])
        train_ds = All_info_loader_noBCLC(Load_feature_t, all_labels_t, patient_file_t, Img_path, resize_Shape=resize_Shape,
                                   transform=train_transforms)
        train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=2, pin_memory=pin_memory)

        val_ds = All_info_loader_noBCLC(Load_feature_v, all_labels_v, patient_file_v, Img_path, resize_Shape=resize_Shape,
                                 transform=None)
        ...
        model = image_clinical_net_v2_noBCLC(1, 20, 1)
        model.to(device)
        model = nn.DataParallel(model, device_ids=device_ids)
        ...
        try:
            while start:
                ...
                for i, data in enumerate(Bar(train_loader), 1):
                    ...
                    for in_i in range(Img.shape[0]):
                        train_out.append(torch.sigmoid(output)[in_i].detach().cpu().numpy())
                        train_label.append(label[in_i].detach().cpu().numpy())
                    loss = criterion(output, label)

                    train_loss += loss.item()
                    loss.backward()
                    optimizer.step()
                train_accuracy, train_sensitivity, train_specificity, train_auc = caculate(train_out, train_label,
                                                                                           cut_off=cut_off)
                writer.add_scalar('train_accuary', train_accuracy, epoch)
                writer.add_scalar('train_loss', train_loss / len(train_loader), epoch)

                del train_out, train_label
                ...
                for j, data_v in enumerate(Bar(val_loader), 1):
                    with torch.no_grad():
                        ...
                        for in_j in range(Img_v.shape[0]):
                            val_out.append(torch.sigmoid(output_v)[in_j].detach().cpu().numpy())
                            val_label.append(label_v[in_j].detach().cpu().numpy())
                        loss_v = criterion(output_v, label_v)

                        val_loss += loss_v.item()

                acc_v, sen_v, spec_v, auc_v = caculate(val_out, val_label, cut_off=cut_off)
                print(f'epoch:{epoch} '
                      f'train_loss:{train_loss / len(train_loader):.4f}, train_acc:{train_accuracy:.3f},',
                      f'train_sen:{train_sensitivity:.2f}, trian_sep: {train_specificity:.2f}, train_acu:{train_auc:.2f}',
                      f'val_loss:{val_loss / len(val_loader):.4f}, val_acc:{acc_v:.4f},',
                      f'val_sen:{sen_v:.2f}, val_sep: {spec_v:.2f}, val_auc:{auc_v:.2f}')

                writer.add_scalar('val_accuary', acc_v, epoch)
...
if __name__ == '__main__':
    ...
    batch_size = 4
    model_name = 'noBCLC_April4th_early20_noscheduler'
    Img_path = 'D:/Yi/HCC_v2/Cropped_Image'
    cut_off = 0.5
    model_save_path = 'D:\Yi\HCC_v2\Train_all_in_April\Train_Image_clinical_radiomics/model_save_Address'
    resize_Shape = [100, 256, 256]
    main(root, batch_size, model_name, Img_path, cut_off, model_save_path, resize_Shape)

    ''''
    -------------------------------------resnet-------------------------------------------
    model_name = resnet121
    model = monai.networks.nets.resnet121(spatial_dims =3, n_input_channels=1, num_classes=1)
    --------------------------------DesNet------------------------------------------------
    ...
    -------------------------------------SENet--------------------------------------------
    model_name = SENet
    model = monai.networks.nets.SENet(spatial_dims =3, in_channels=1, num_classes=1)
    ---------------------------------------VIT--------------------------------------------
    model_name = ViT
    model = monai.networks.nets.ViT(img_size = [100, 256, 256], patch_size= [25, 32, 32], spatial_dims =3, in_channels=1, num_classes=1, pos_embed='conv',classification=True)
    out = model(input)[0]
    '''
