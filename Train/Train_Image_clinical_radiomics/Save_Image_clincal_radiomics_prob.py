# -*- coding: utf-8 -*-
# @Time    : 18-4-2024  15:39
# @Author  : Yi Chen
# @File    : Save_Image_clincal_radiomics_prob.py
# @Software: PyCharm
# @Describe: 
# -*- encoding:utf-8 -*-
# -*- coding: utf-8 -*-
...
def main(root, batch_size, model_name, Img_path, cut_off, save_path, resize_Shape):
    ...
    for i in range(10):
        ...
        train_ds = All_info_loader_noBCLC(Load_feature_t, all_labels_t, patient_file_t, Img_path,
                                          resize_Shape=resize_Shape,
                                          transform=train_transforms)
        train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=2, pin_memory=pin_memory)

        val_ds = All_info_loader_noBCLC(Load_feature_v, all_labels_v, patient_file_v, Img_path,
                                        resize_Shape=resize_Shape,
                                        transform=None)
        ...
        model.to(device)
        model = nn.DataParallel(model, device_ids=device_ids)
        model.load_state_dict(torch.load(os.path.join(model_Save_address, 'checkpoints', 'checkpoint.pth')))
        ...
        with torch.no_grad():
            ...
            for i_t, data in enumerate(Bar(train_loader), 1):
                ...
                for batch in range(output.shape[0]):
                    pred_prob.append(torch.sigmoid(output[batch, 0]).detach().cpu().numpy())
            ...
            for j_v, data_v in enumerate(Bar(val_loader), 1):
                ...
                for batch in range(output_v.shape[0]):
                    val_prob.append(torch.sigmoid(output_v[batch, 0]).detach().cpu().numpy())
...
if __name__ == '__main__':
    ...
    batch_size = 4
    model_name = 'noBCLC_April4th_early10_noscheduler'
