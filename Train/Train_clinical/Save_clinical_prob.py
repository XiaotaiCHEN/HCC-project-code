...
def main(root, batch_size, model_name, Img_path, cut_off, save_path, resize_Shape):
    ...
    for i in range(10):
        ...
        with torch.no_grad():
            ...
            df_t = pd.DataFrame(np.array(prob_t).T, columns=[f'Column{i + 1}' for i in range(np.array(prob_t).shape[0])], index=['Lille', 'prob', 'label'])
            ...
            for j_v, data_v in enumerate(Bar(val_loader), 1):
                ...
                for batch in range(output_v.shape[0]):
                    ...
                    prob_v.append([patient_ID[batch], torch.sigmoid(output_v[batch, 0]).detach().cpu().numpy(),
                                 label_v[batch].detach().cpu().numpy()])
        ...
        df = pd.DataFrame(np.array(prob_v).T, columns=[f'Column{i + 1}' for i in range(np.array(prob_v).shape[0])], index=['Lille', 'prob', 'label'])
...
if __name__ == '__main__':
    ...
    batch_size = 4
    model_name = 'Linear_early15_March25'
