...
from Train_all_in_April.Train_Image.Image_dataloader import All_info_loader, load_all_information
...
def main(root, batch_size, model_name, Img_path, cut_off, save_path, resize_Shape):
    ...
    for i in range(10):
        os.makedirs(
            ...
            exist_ok=True)
        model_Save_address = os.path.join(save_path, model_name, 'Fold_' + str(i), 'checkpoint', 'checkpoint.pth')
        ...
        all_labels_t, patient_file_t = load_all_information(root, train_patient_list)
        all_labels_v, patient_file_v = load_all_information(root, val_patient_list)

        train_transforms_new = Compose([
        ...
        train_ds = All_info_loader(all_labels_t, patient_file_t, Img_path, resize_Shape=resize_Shape,
        ...
        val_ds = All_info_loader(all_labels_v, patient_file_v, Img_path, resize_Shape=resize_Shape,
                                 transform=None)
        ...
        model = monai.networks.nets.resnet50(spatial_dims=3, n_input_channels=1, num_classes=1)

        model.to(device)
        model = nn.DataParallel(model, device_ids=device_ids)

        # optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)  # 0.000928061399619526
        ...
        try:
            while start:
                ...
                for i, data in enumerate(Bar(train_loader), 1):
                    ...
                    optimizer.zero_grad()
                    output = model(Img)
                    for lens in range(output.shape[0]):
                        out.append(torch.sigmoid(output[lens, 0]).detach().cpu().numpy())
                        label_t.append(label[lens].detach().cpu().numpy())
                    ...
                    loss.backward()
                    optimizer.step()
                fpr_t, tpr_t, thresholds_t = roc_curve(np.array(label_t), np.array(out))
                ...
                for j, data_v in enumerate(Bar(val_loader), 1):
                    with torch.no_grad():
                        Img = data_v['Image'].type('torch.cuda.FloatTensor').to(device)
                        label_val = data_v['label'].type('torch.cuda.FloatTensor').to(device)
                        patient_ID = data_v['Patient_ID']

                        output_val = model(Img)
                        for lens_v in range(output_val.shape[0]):
                            out_v.append(torch.sigmoid(output_val[lens_v, 0]).detach().cpu().numpy())
                            label_v.append(label_val[lens_v].detach().cpu().numpy())
                        loss_v = criterion(output_val, label_val)
                        acc_v, sen_v, spec_v = caculate(output_val, label_val, cut_off=cut_off)
                ...
                fpr_v, tpr_v, thresholds_v = roc_curve(np.array(label_v), np.array(out_v))
...
if __name__ == '__main__':
    ...
    batch_size = 4
    model_name = 'resnet50_shceduler_March25_Adamlr3_early20_train_transforms_new'
    Img_path = 'D:/Yi/HCC_v2/Cropped_Image'
    cut_off = 0.5
    model_save_path = 'D:/Yi/HCC_v2/Train_all_in_April/Train_Image/model_save_Address'
    ...
    -----------------------------EfficientNet----------------------------------------------
    model_name = 'EfficientNetB0'  ##!!!!need retrain the first two fold
